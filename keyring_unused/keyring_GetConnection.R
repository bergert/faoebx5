...
GetConnection_keyring <- function() {
  ...
  if (nrow(users_ebx_lock) > 0) {
    return(c(connection, secret=keyring::key_get("EBX_SECRET", username = username, keyring = "EBX")))
  }
  users_ebx_unlocked <- keyring::key_list(service = "EBX_SECRET")
  if (nrow(users_ebx_unlocked) > 0) {
    return(c(connection, secret=keyring::key_get("EBX_SECRET", username = connection$username)))
  }
